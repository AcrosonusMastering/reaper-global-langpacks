...
FICHIER_REAPER = "LangPack_TURKISH_SYNCED_reaper_778.txt"  # <-- à adapter

# Fichier SWS (à coller à la suite). Mets le nom exact (même dossier que le script)
FICHIER_SWS = "SWS_TURKISH.txt"  # <-- à adapter
...
def nettoyer_retours_ligne_litteraux(modifs: Dict[int, str]) -> Dict[int, str]:
    """
    Garde-fou anti-bug récurrent.

    Dans le format LangPack REAPER, un saut de ligne À L'INTÉRIEUR d'un texte
    (ex: message de dialogue multi-lignes) est toujours représenté par la
    séquence LITTÉRALE \\r\\n ou \\n (2 ou 4 caractères) sur UNE SEULE ligne
    physique du fichier. Une entrée de MES_MODIFS ne doit donc JAMAIS contenir
    un vrai caractère de retour chariot/saut de ligne (\r ou \n réels) : si
    c'est le cas, écrire le fichier de sortie tel quel casserait cette ligne
    en plusieurs lignes physiques (et donc le nombre total de lignes du
    fichier).

    Cause typique : dans le code Python, écrire "\\r\\n" (antislash simple)
    au lieu de "\\\\r\\\\n" (antislash doublé) ou d'une chaîne brute r"...".

    Cette fonction détecte ces cas, corrige automatiquement (en réinsérant la
    séquence littérale attendue) et affiche une alerte listant les clés
    concernées, AVANT l'écriture du fichier de sortie.
    """
    modifs_ok = {}
    lignes_corrigees = []

    for num_ligne, valeur in modifs.items():
        if "\r" in valeur or "\n" in valeur:
            valeur_corrigee = valeur.replace("\r\n", "\\r\\n").replace("\r", "\\r").replace("\n", "\\n")
            modifs_ok[num_ligne] = valeur_corrigee
            lignes_corrigees.append(num_ligne)
        else:
            modifs_ok[num_ligne] = valeur

    if lignes_corrigees:
        noms = ", ".join(str(n) for n in sorted(lignes_corrigees))
        log(f"[ALERTE] Retour(s) à la ligne réel(s) détecté(s) et corrigé(s) automatiquement "
            f"dans MES_MODIFS pour {len(lignes_corrigees)} entrée(s) : {noms}")
        log("          -> Cause probable : \\r\\n ou \\n écrit avec un antislash simple dans le "
            "script au lieu d'un antislash doublé (\\\\r\\\\n) ou d'une chaîne brute r\"...\".")
        log("          -> Ces entrées ont été réécrites pour rester sur UNE SEULE ligne physique, "
            "comme l'exige le format LangPack REAPER. Vérifie le fichier de sortie sur ces lignes.")
...
def operation_replace(contenu_reaper: str, modifs: Dict[int, str]) -> Tuple[str, int, int]:
    ...
    modifs = nettoyer_retours_ligne_litteraux(modifs)
