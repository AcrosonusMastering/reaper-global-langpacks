...
LANGUAGES = {
     ...
     "SERBOCROATIAN": Path("SERBOCROATIAN_773.txt"),   
}
...
def template_tag(template_path=None):
    # Build version tag from template filename stem, e.g. "reaper_769"
    # template_path lets callers (e.g. merge_one, CLI mode) use the template
    # they actually received instead of silently falling back to the global
    # TEMPLATE_FILE config, which was the source of a tag/filename mismatch bug.
    stem = (template_path if template_path is not None else TEMPLATE_FILE).stem
    tag = sanitize_tag(stem)
    return tag if tag else "template"

def maybe_tag_suffix(template_path=None):
    return "_" + template_tag(template_path) if INCLUDE_TEMPLATE_TAG_IN_FILENAMES else ""
...
"""
    ...
    tag = template_tag(template_path)
    parts = []
...
def merge_one(template_path, source_path, lang_out_dir, code):
    ...
    suffix = maybe_tag_suffix(template_path)
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) >= 6:
            ...
            # IMPORTANT: use the tag for the template actually passed on the CLI
            # (not the global TEMPLATE_FILE) - merge_one() now does the same
            # internally, so this matches exactly what it just wrote to disk.
            suffix = maybe_tag_suffix(template)
            (tmp_dir / f"LangPack_{tmp_code}_SYNCED{suffix}.txt").replace(outp)
            (tmp_dir / f"LangPack_{tmp_code}_ADDED{suffix}.txt").replace(added)
            (tmp_dir / f"LangPack_{tmp_code}_REMOVED{suffix}.txt").replace(removed)
            (tmp_dir / f"LangPack_{tmp_code}_REPORT{suffix}.html").replace(report)
